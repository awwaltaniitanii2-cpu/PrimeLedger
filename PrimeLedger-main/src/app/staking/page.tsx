    ...
    <PageShell>
      <div className="space-y-8 pb-12">
        <GlassPanel className="p-8 lg:p-10">
          <SectionHeader
            eyebrow="PrimeLedger Staking"
            title="Digital Asset Staking"
            description="Stake supported digital assets through professionally managed PrimeLedger staking products with transparent APR, duration, and capacity."
        ...
        {products.length === 0 ? (
          <GlassPanel className="p-10">
            <h2 className="text-4xl font-black tracking-[-0.04em] text-white">
              No Active Staking Products
            </h2>

            <p className="mt-4 max-w-2xl text-[#7F8DA3]">
              Staking opportunities created by administrators will appear here.
            </p>
          </GlassPanel>
        ) : (
          <section className="grid gap-6 xl:grid-cols-3">
            {products.map((product) => {
              ...
              const progress =
                capacity > 0
                  ? Math.min((invested / capacity) * 100, 100)
                  : 0;
                  ...
                  {product.totalCapacity ? (
                    <div className="mt-8 rounded-[26px] border border-white/10 bg-black/25 p-5">
                      ...
                      <p className="mt-3 text-sm text-[#7F8DA3]">
                        ...
                        {capacity.toLocaleString()} {product.currency}
                      </p>
                    </div>
                  ) : null}

                  <GlowButton
                    href={`/staking/${product.id}`}
